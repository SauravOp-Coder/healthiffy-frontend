...
const CustomerMenu = () => {
  ...
  const handleCheckout = async (method) => {
    ...
    try {
        ...
        if (method === 'cash') {
            ...
            // Trigger the app opening automatically
            setTimeout(() => {
        ...
        <div style={verifyOverlay}>
            <div style={verifyCard}>
                <div style={loaderWrapper}>
                   <Loader2 size={40} className="spin-icon" color="#f39c12" />
                </div>
                ...
                <div style={upiDetailsBox}>
                    <p style={{margin: '0', fontSize: '0.85rem', color: '#555'}}>
                        Order <b>#{shortId}</b> is placed. <br/>
                        Stay here while the kitchen confirms payment.
                    </p>
                </div>
  ...
  return (
    <div style={container}>
      {/* Verification Overlay - THIS CALLS THE FUNCTION ABOVE */}
...
const loaderWrapper = { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' };
const verifyTitle = { fontSize: '1.5rem', fontWeight: '800', color: '#1a1a1a', margin: '0 0 10px 0' };
const verifySubtitle = { fontSize: '0.9rem', color: '#666', marginBottom: '20px' };
const upiDetailsBox = { background: '#f8fafc', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', textAlign: 'left', marginBottom: '20px' };
const backBtn = { background: 'none', border: '1px solid #e2e8f0', padding: '10px 20px', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', color: '#64748b', fontSize: '0.9rem' };
const qrContainer = { background: 'white', padding: '15px', borderRadius: '15px', display: 'inline-block', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', marginBottom: '20px', marginTop: '10px' };
